...
% Quote ML_ROOT if it contains spaces
if contains(mlRoot, ' ')
...
if ispc
    ...
    glfwIncPath = getUnixPath( getpref('mujoco', 'glfwIncPath'));
    glfwImportLibPath = getUnixPath( getpref('mujoco', 'glfwImportLibPath'));
    % Quote paths if they contain spaces
    if contains(glfwIncPath, ' ')
        fprintf(fileID, 'SET_GLFW_INC_PATH=-I"%s"\n', glfwIncPath);
    else
        fprintf(fileID, "SET_GLFW_INC_PATH=-I%s\n", glfwIncPath);
    end
    if contains(glfwImportLibPath, ' ')
