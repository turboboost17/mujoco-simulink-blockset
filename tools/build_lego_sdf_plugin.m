function artifact = build_lego_sdf_plugin(opts)
%BUILD_LEGO_SDF_PLUGIN Build and optionally install the Lego SDF MuJoCo plugin.
...
pluginSourceDir = fullfile(repoRoot, 'plugins', 'lego_sdf');
...
if ~isfolder(mujocoRoot)
    error('legoSdf:missingMujocoRoot', 'MuJoCo root does not exist: %s', mujocoRoot);
end

if ~isfile(fullfile(mujocoRoot, 'include', 'mujoco', 'mujoco.h'))
    error('legoSdf:missingMujocoHeaders', 'MuJoCo headers not found under: %s', mujocoRoot);
end

if ispc
    localBuildWithMsvc(pluginSourceDir, buildDir, mujocoRoot, opts.Config);
else
    configureCmd = sprintf('cmake -S %s -B %s -DMUJOCO_ROOT=%s -DCMAKE_BUILD_TYPE=%s -DLEGO_SDF_BUILD_SMOKE=ON', ...
...
if ispc
    artifact = fullfile(buildDir, opts.Config, 'lego_sdf.dll');
else
    artifact = fullfile(buildDir, 'liblego_sdf.so');
end

if ~isfile(artifact)
    error('legoSdf:missingArtifact', 'Expected plugin artifact was not built: %s', artifact);
end
...
fprintf('Lego SDF plugin ready: %s\n', artifact);
end
...
if isempty(compiler)
    error('legoSdf:noCompiler', 'No selected C++ compiler. Run mex -setup C++ first.');
end

vcvars = fullfile(compiler.Location, 'VC', 'Auxiliary', 'Build', 'vcvars64.bat');
if ~isfile(vcvars)
    error('legoSdf:noVcvars', 'Could not find vcvars64.bat under compiler location: %s', compiler.Location);
end
...
scriptFile = fullfile(outputDir, 'build_lego_sdf.bat');
fileId = fopen(scriptFile, 'w');
if fileId < 0
    error('legoSdf:buildScriptOpen', 'Could not open build script for writing: %s', scriptFile);
end
...
fprintf(fileId, 'cl /nologo /LD /EHsc /std:c++17 /O2 %s %s "%s" "%s" /link %s mujoco.lib /OUT:"%s" /IMPLIB:"%s"\r\n', ...
    includeFlag, objectDirFlag, fullfile(pluginSourceDir, 'lego_brick.cc'), ...
    fullfile(pluginSourceDir, 'register.cc'), libPathFlag, ...
    fullfile(outputDir, 'lego_sdf.dll'), fullfile(outputDir, 'lego_sdf.lib'));
fprintf(fileId, 'if errorlevel 1 exit /b %%errorlevel%%\r\n');
...
fprintf('%s', output);
if status ~= 0
    error('legoSdf:commandFailed', 'Command failed with status %d: %s', status, commandText);
end
end
