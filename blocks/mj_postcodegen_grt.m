...
% GLFW is linked from the system package (libglfw3-dev) on the target.
    ...
    patchRos2PackageXml(buildInfo, {'libglfw3-dev', 'libgl-dev'});
end
...
% Patch generated CMakeLists.txt so colcon installs runtime assets.
    ...
    existingFiles = findRos2PackageFiles(modelName, 'CMakeLists.txt');
...
function patchRos2PackageXml(buildInfo, dependencyNames)
% Patch generated package.xml with target system dependencies.
% MuJoCo shared libraries are copied and installed by CMake rules because
% libmujoco.so is bundled with this blockset rather than resolved by rosdep.
    modelName = buildInfo.ComponentName;
    packageFiles = findRos2PackageFiles(modelName, 'package.xml');

    if isempty(packageFiles)
        warning('mujoco:postcodegen:noPackageXml', ...
            'Could not find package.xml to patch ROS dependencies for %s.', ...
            lower(modelName));
        return
    end

    marker = '<!-- MuJoCo dependencies (auto-generated by mj_postcodegen_grt) -->';
    dependencyBlock = sprintf('\n  %s\n', marker);
    for i = 1:numel(dependencyNames)
        dependencyBlock = [dependencyBlock, sprintf( ...
            '  <build_depend>%s</build_depend>\n  <exec_depend>%s</exec_depend>\n', ...
            dependencyNames{i}, dependencyNames{i})]; %#ok<AGROW>
    end

    for i = 1:numel(packageFiles)
        packageFile = packageFiles{i};
        txt = fileread(packageFile);
        if contains(txt, marker)
            continue
        end
        if ~contains(txt, '</package>')
            warning('mujoco:postcodegen:badPackageXml', ...
                'Could not find </package> in %s.', packageFile);
            continue
        end
        txt = strrep(txt, '</package>', [dependencyBlock, '</package>']);
        fid = fopen(packageFile, 'w');
        if fid < 0
            warning('mujoco:postcodegen:packageOpenFailed', ...
                'Could not open %s to add MuJoCo dependencies.', packageFile);
            continue
        end
        cleanup = onCleanup(@() fclose(fid));
        fprintf(fid, '%s', txt);
        delete(cleanup);
        fprintf('Patched %s with MuJoCo ROS dependencies.\n', packageFile);
    end
end

function existingFiles = findRos2PackageFiles(modelName, fileName)
    packageName = lower(modelName);
    candidates = { ...
        fullfile(pwd, '..', packageName, fileName), ...
        fullfile(pwd, '..', '..', 'src', packageName, fileName), ...
        fullfile(pwd, '..', 'src', packageName, fileName) ...
    };
