...
#pragma once
#endif
#ifndef SIMSTRUC_IMPLEMENT_H
#define SIMSTRUC_IMPLEMENT_H
...
#define _ssSafelyCallConstGenericFcnStart(S) \
       (((S)->mdlInfo->genericFcn==SS_NULL) ? 0 : ((*((_ConstGenericFcn)(S)->mdlInfo->genericFcn))
#define _ssSafelyCallGenericFcnEnd ))

#define _ssSafelyCallGenericFcnStart(S) \
       (((S)->mdlInfo->genericFcn==SS_NULL) ? 0 : ((*(S)->mdlInfo->genericFcn)
#define _ssSafelyCallGenericFcnEnd ))
...
#define dtaGetDataTypeSize(dta, blockPath, id)                                               \
    ((dta) != SS_NULL ? (dta)->getGenericDTAIntProp((dta)->dataTypeTable, (blockPath), (id), \
...
#define ssComputeInput(S, pIdx)                                              \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_COMPUTE_INPUT, pIdx, SS_NULL) \
...
#if SS_SIM
#define dtaSetErrorString(dta, string) ((dta) != SS_NULL ? (dta)->errorString = (string) : 0)
#else
...
#define dtaSetIsPositiveFcn(dta, blockPath, id, fcn)                                            \
    ((dta) != SS_NULL ? (dta)->setGenericDTAUnaryFcnGW((dta)->dataTypeTable, (blockPath), (id), \
...
#define dtaGetDataTypeElementOffset(dta, blockPath, id, eIdx)                                    \
    ((dta) != SS_NULL ? (dta)->getGenericDTAIntElemProp((dta)->dataTypeTable, (blockPath), (id), \
...
#if SS_SIM
#define dtaGetErrorString(dta) ((dta) != SS_NULL ? (dta)->errorString : SS_NULL)
#else
...
#define dtaRegisterDataType(dta, blockPath, name)                                     \
    ((dta) != SS_NULL ? (dta)->registerFcn((dta)->dataTypeTable, (blockPath), (name)) \
                      : INVALID_DTYPE_ID)
#else
...
#define dtaGetNumDataTypes(dta) \
    ((dta) != SS_NULL ? (dta)->getNumDataTypesFcn((dta)->dataTypeTable) : INVALID_NUM_DTYPES)
#else
...
#define dtaGetDataTypeId(dta, name) \
    ((dta) != SS_NULL ? (dta)->getIdFcn((dta)->dataTypeTable, (name)) : INVALID_DTYPE_ID)
#else
...
#define dtaGetDataTypeName(dta, blockPath, id)                                                \
    ((dta) != SS_NULL ? (dta)->getGenericDTAVoidProp((dta)->dataTypeTable, (blockPath), (id), \
...
#define dtaSetDataTypeSize(dta, blockPath, id, size)                                         \
    ((dta) != SS_NULL ? (dta)->setGenericDTAIntProp((dta)->dataTypeTable, (blockPath), (id), \
...
#define dtaSetDataTypeZero(dta, blockPath, id, zero)                                          \
    ((dta) != SS_NULL ? (dta)->setGenericDTAVoidProp((dta)->dataTypeTable, (blockPath), (id), \
...
#else
#define dtaGetDataTypeElementDataType(dta, blockPath, id, eIdx)                            \
    ((dta) != SS_NULL && (dta->access64) != SS_NULL                                        \
         ? (dta->access64)                                                                 \
...
#define dtaGetDataTypeElementDimensions(dta, blockPath, id, eIdx)            \
    ((dta) != SS_NULL ? ((const int_T*)(dta)->getGenericDTAVoidElemProp(     \
...
#define dtaGetDataTypeElementMin(dta, blockPath, id, eIdx)                                        \
    ((dta) != SS_NULL ? (dta)->getGenericDTARealElemProp((dta)->dataTypeTable, (blockPath), (id), \
...
#define dtaGetConvertBetweenFcn(dta, blockPath, id)                                          \
    ((dta) != SS_NULL ? (dta)->getConvertBetweenFcn((dta)->dataTypeTable, (blockPath), (id)) \
                      : SS_NULL)
#else
...
#define dtaCallConvertBetweenForSrcId(dta, blockPath, dstId, srcId, numEls, u, options, y) \
    (dtaGetConvertBetweenFcn((dta), (blockPath), (srcId)) != SS_NULL                       \
...
#define dtaCallConvertBetweenForDstId(dta, blockPath, dstId, srcId, numEls, u, options, y) \
    (dtaGetConvertBetweenFcn((dta), (blockPath), (dstId)) != SS_NULL                       \
...
#define dtaGetConstructFcn(dta, blockPath, id) \
    ((dta) != SS_NULL ? (dta)->getConstructFcn((dta)->dataTypeTable, (blockPath), (id)) : SS_NULL)
#else
...
#define dtaSetConstructFcn(dta, blockPath, id, fcn) \
    ((dta) != SS_NULL ? (dta)->setConstructFcn((dta)->dataTypeTable, (blockPath), (id), (fcn)) : 0)
#else
...
#define dtaCallConstruct(dta, blockPath, id, u)              \
    (dtaGetConstructFcn((dta), (blockPath), (id)) != SS_NULL \
...
#define dtaGetDestructFcn(dta, blockPath, id) \
    ((dta) != SS_NULL ? (dta)->getDestructFcn((dta)->dataTypeTable, (blockPath), (id)) : SS_NULL)
#else
...
#define dtaSetDestructFcn(dta, blockPath, id, fcn) \
    ((dta) != SS_NULL ? (dta)->setDestructFcn((dta)->dataTypeTable, (blockPath), (id), (fcn)) : 0)
#else
...
#define dtaCallDestruct(dta, blockPath, id, u)              \
    (dtaGetDestructFcn((dta), (blockPath), (id)) != SS_NULL \
...
#define dtaGetDeepCopyFcn(dta, blockPath, id) \
    ((dta) != SS_NULL ? (dta)->getDeepCopyFcn((dta)->dataTypeTable, (blockPath), (id)) : SS_NULL)
#else
...
#define dtaSetDeepCopyFcn(dta, blockPath, id, fcn) \
    ((dta) != SS_NULL ? (dta)->setDeepCopyFcn((dta)->dataTypeTable, (blockPath), (id), (fcn)) : 0)
#else
...
#define dtaCallDeepCopy(dta, blockPath, id, y, u)                \
    (dtaGetDeepCopyFcn((dta), (blockPath), (id)) != SS_NULL      \
...
#define dtaGetSerializeFcn(dta, id) \
    ((dta) != SS_NULL ? (dta)->getSerializeFcn((dta)->dataTypeTable, (id)) : SS_NULL)
#else
...
#define dtaSetSerializeFcn(dta, id, fcn) \
    ((dta) != SS_NULL ? (dta)->setSerializeFcn((dta)->dataTypeTable, (id), (fcn)) : 0)
#else
...
#define dtaCallSerialize(dta, id, y, u, w, o)                  \
    (dtaGetSerializeFcn((dta), (id)) != SS_NULL                \
...
#define dtaGetDeserializeFcn(dta, id) \
    ((dta) != SS_NULL ? (dta)->getDeserializeFcn((dta)->dataTypeTable, (id)) : SS_NULL)
#else
...
#define dtaSetDeserializeFcn(dta, id, fcn) \
    ((dta) != SS_NULL ? (dta)->setDeserializeFcn((dta)->dataTypeTable, (id), (fcn)) : 0)
#else
...
#define dtaCallDeserialize(dta, id, y, u, w, o)                  \
    (dtaGetDeserializeFcn((dta), (id)) != SS_NULL                \
...
#define dtaGetSerializeSizeFcn(dta, id) \
    ((dta) != SS_NULL ? (dta)->getSerializeSizeFcn((dta)->dataTypeTable, (id)) : SS_NULL)
#else
...
#define dtaSetSerializeSizeFcn(dta, id, fcn) \
    ((dta) != SS_NULL ? (dta)->setSerializeSizeFcn((dta)->dataTypeTable, (id), (fcn)) : 0)
#else
...
#define dtaCallSerializeSize(dta, id, y, u)              \
    (dtaGetSerializeSizeFcn((dta), (id)) != SS_NULL      \
...
#define dtaGetIsPositiveFcn(dta, blockPath, id)                                                 \
    ((dta) != SS_NULL ? (dta)->getGenericDTAUnaryFcnGW((dta)->dataTypeTable, (blockPath), (id), \
...
#define dtaCallIsPositive(dta, blockPath, id, numEls, u, options, y)                              \
    (dtaGetIsPositiveFcn((dta), (blockPath), (id)) != SS_NULL                                     \
...
#define dtaCallIsNegative(dta, blockPath, id, numEls, u, options, y)                              \
    (dtaGetIsNegativeFcn((dta), (blockPath), (id)) != SS_NULL                                     \
...
#define dtaGetGreaterThanFcn(dta, blockPath, id)                                                 \
    ((dta) != SS_NULL ? (dta)->getGenericDTABinaryFcnGW((dta)->dataTypeTable, (blockPath), (id), \
...
#define dtaSetGreaterThanFcn(dta, blockPath, id, fcn)                                            \
    ((dta) != SS_NULL ? (dta)->setGenericDTABinaryFcnGW((dta)->dataTypeTable, (blockPath), (id), \
...
#define dtaCallGreaterThan(dta, blockPath, id, numEls, u1, u2, options, y)                    \
    (dtaGetGreaterThanFcn((dta), (blockPath), (id)) != SS_NULL                                \
...
#define dtaCallGreaterEqual(dta, blockPath, id, numEls, u1, u2, options, y)                    \
    (dtaGetGreaterEqualFcn((dta), (blockPath), (id)) != SS_NULL                                \
...
#define dtaGetDataOverflowDiagnostic(dta, blockPath, result)                              \
    ((dta) != SS_NULL ? (dta)->getGenericDTADiagnostic((dta)->dataTypeTable, (blockPath), \
...
#define ssGetIsContModeFrozen(S) \
    ((S)->mdlInfo->solverInfo->isContModeFrozen == 1U) /* (boolean_T) */

#define ssSetIsContModeFrozen(S, val) ssGetSolverInfo(S)->isContModeFrozen = (val)

#endif /* SIMSTRUC_IMPLEMENT_H */

#ifdef __clang__
#pragma clang diagnostic pop
#endif

/* LocalWords:  Wunused Wreserved Wzero NRT
 */
