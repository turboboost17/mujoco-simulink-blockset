...
struct _ssMdlInfo {
    ...
    struct {
        ...
        unsigned int zcEvalForRefine         : 1; /* if zc function is being evaluated on
                                                     refine */
        unsigned int isModelAbsTolAuto       : 1; /* is get_param(model,'AbsTol') == 'auto' */
        unsigned int isModelAbsTolScaled : 1; /* is get_param(model,'AutoScaleAbsTol') == 'on' */

        unsigned int reserved6           : 6; /* remainder are reserved      */
    } mdlFlags;
